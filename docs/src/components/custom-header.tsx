...
export function CustomHeader() {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-40 border-b border-white/10 bg-[#0b0d10]/92 backdrop-blur-xl">
      <div className="container mx-auto flex h-14 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <div className="relative flex items-center" ref={dropdownRef}>
            <Link
              href="/"
              className="flex items-center gap-2 rounded-xl border border-transparent px-2 py-1.5 text-white transition-colors hover:border-white/10 hover:bg-white/5"
            >
              ...
              <span className="font-medium text-[var(--brand-500)]">{currentSite.label}</span>
            </Link>
            <button
            ...
            {/* Dropdown menu */}
            {isOpen && (
              <div className="absolute left-0 top-full mt-2 w-72 rounded-2xl border border-white/10 bg-[var(--bg-elev-1)] p-1.5 shadow-[0_24px_64px_rgba(0,0,0,0.45)]">
                {docsSites.map((site, index) => {
                  const isActive = site.name === currentSite.name;
                  return (
                    <div key={site.name}>
                      {index > 0 && <div className="mx-2 my-1 border-t border-white/8" />}
                      <Link
                        href={site.href}
                        ...
                        className={cn(
                          ...
                          isActive
                            ? 'border-[rgba(97,188,255,0.22)] bg-[rgba(97,188,255,0.12)] text-[var(--ink-strong)]'
                            : 'border-transparent text-[var(--ink-muted)] hover:border-white/10 hover:bg-[var(--bg-elev-2)] hover:text-[var(--ink-strong)]'
                        )}
                      >
                        ...
                        <div className="flex-1">
                          <div className="font-medium text-[var(--ink-strong)]">{site.name}</div>
                          <div className="text-xs text-[var(--ink-muted)]">{site.description}</div>
                        </div>
                        {isActive && <Check className="h-4 w-4 text-[var(--brand-500)]" />}
                      </Link>
                    </div>
                  );
                })}
